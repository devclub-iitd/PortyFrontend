...
class LanguageExpansionPanel extends React.Component {
  constructor(props) {
    ...
    for (let i = 0; i < existingData.length; i += 1) {
      tempFields.push(<LanguageDetails data={existingData[0]} handleChange={this.handleInputChange} key={i} id={i} expanded={expanded} action={() => this.handlePanel(`languagePanel${i}`)} moveFieldDown={() => this.moveFieldDown(i, i)} moveFieldUp={() => this.moveFieldUp(i, i)} />);
  ...
  handleInputChange(event) {
    ...
    const { language, languageFieldTracker, languageDetailsCount, expanded } = this.state;
